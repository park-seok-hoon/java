...
public class ListEx2 {
	...
	//정수이기 떄문에 Integer를 추가. int가 안되는 이유는 제네릭은 클래스만 올 수 있어서
		...
		//입력받은 정수의 개수
		int count = 5;
		//system에 있는 out 인스턴스에서 제공하는 print메서드를 이용하여 콘솔창에 문자열 출력
		
		System.out.println("정수 5개를 입력하세요 : ");
		//5번 반복하기 위해 반복문을 작성
		//i는 0부터 4까지 5번 반복
		for(int i=0; i<count; i++) 
			//scan.nextInt()릁 통해 입력 버퍼에 있는 값 중 정수 값을 가져옴.
			//만약 입력 버퍼에 정수가 아님 문자나 문자열이 있으면 예외가 발생!!
			//가져온 정수를 tmp에 저장
		{   int tmp = sc.nextInt();
